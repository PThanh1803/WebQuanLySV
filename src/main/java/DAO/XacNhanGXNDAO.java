...
public class XacNhanGXNDAO {
	...
	private static final String Select_All_Request_GXN = "SELECT d.ID, d.ThoiGianDki, d.MSSV, l.TenLoaiGiay, d.SoLuong " +
            "FROM dki_giay_xac_nhan d " +
            "LEFT JOIN loai_giay l ON d.MaLoaiGiay = l.MaLoaiGiay " +
            "WHERE d.TrangThai = 'Chưa duyệt'";
	
	private static final String Select_All_Request_GXNID = "SELECT d.ID, d.ThoiGianDki, d.MSSV, l.TenLoaiGiay, d.SoLuong " +
            "FROM dki_giay_xac_nhan d " +
            "LEFT JOIN loai_giay l ON d.MaLoaiGiay = l.MaLoaiGiay " +
            "WHERE d.TrangThai = 'Chưa duyệt' AND d.MaLoaiGiay = ?";
	
	private static final String Select_Request_GXN_ID = "SELECT d.ThoiGianDKi, d.MSSV, l.TenLoaiGiay, d.SoLuong " +
            "FROM dki_giay_xac_nhan d " +
            "LEFT JOIN loai_giay l ON d.MaLoaiGiay = l.MaLoaiGiay " +
            "WHERE d.ID = ?";
	...
	public List<XacNhanGXN> SelectAllRequestsGXN(String maLoaiGiay) {
        ...
        try (Connection connection = JDBCUtil.getConnection();
            ...
            while (rs.next()) {
                ...
                gxns.add(new XacNhanGXN(ID, tenLoaiGiay, mssv, thoigiandangki, soLuong, trangThai));
            }
